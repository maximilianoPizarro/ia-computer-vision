...
. ApplicationSet: `charts/all/spoke-neuroface/templates/applicationset-user-neuroface.yaml`
...
$ oc get application -n vp-gitops | grep neuroface-user
$ oc get httproute,authpolicy -n neuroface-user1
----

.Expected: ApplicationSet generates one Argo CD Application per `neuroface-*` GitLab repo; HTTPRoute and AuthPolicy exist in the user namespace.
