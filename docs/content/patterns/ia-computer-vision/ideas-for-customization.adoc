...
<2> Scales workshop user counts and replica counts down so the full stack fits a single node's pod-count ceiling. See link:../cluster-sizing/[Cluster sizing].
<3> Adapts the pattern to a cluster where NFD, the GPU Operator, Service Mesh, OpenShift AI, and Connectivity Link are already installed by someone else — this section's focus.
<4> Optional. Only needed for a hub-only install with no east or west spoke clusters. Omit it if you plan to add spokes.
...
* `values-hub-rhpds.yaml` — `OperatorGroup` skip list, ZTunnel version pin, Connectivity Link namespace, manual install-plan auto-approval, and `ssoHostPrefix` overrides
