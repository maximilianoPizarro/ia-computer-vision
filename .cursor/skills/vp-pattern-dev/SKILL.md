...
### Unresolved: hub `spec.rbac.policy`/`defaultPolicy`/`scopes` on `ArgoCD/vp-gitops` reverts within &lt;1s even with zero competing charts
After fixing the dual-chart ownership race below, `spec.localUsers` (the `ai-agent` entry) sticks reliably and the operator issues a real, working API token (`ai-agent-local-user` Secret; confirmed live with a `curl` read call returning HTTP 200 for `GET /api/v1/applications`). **However** `spec.rbac.policy`/`defaultPolicy`/`scopes` keeps reverting to a stale value (`defaultPolicy: role:readonly`, 3-line policy without the `role:ai-agent` grant, `scopes: [groups,email]`) in well under 1 second after every `oc patch`/Argo CD sync — confirmed by immediate polling and by patching `argocd-rbac-cm` directly (also reverts). Ruled out during investigation: no competing Argo CD Application (`argocd-local-users` deleted, app-of-apps `ia-computer-vision-hub` confirmed synced past its removal); no mutating/validating webhook targets `argoproj.io` resources (audited all 34 `MutatingWebhookConfiguration`s cluster-wide); restarting `openshift-gitops-operator-controller-manager` did not change the behavior. Field-manager audit (`oc get argocd vp-gitops -o yaml --show-managed-fields`) shows a generic `manager` identity (consistent with a controller-runtime operator's default field-manager name) claiming `spec.rbac` ownership at the same instant Argo CD's own `argocd-controller` SSA apply succeeds — pointing at `openshift-gitops-operator`'s own reconcile loop enforcing some cached/derived RBAC baseline, but the actual source was not found within this session (no `ArgoCDExport`/backup CR exists; no cluster-wide ConfigMap/Secret found holding the stale string besides `argocd-rbac-cm` itself, which the operator also keeps rewriting). **Practical impact**: `ai-agent` can currently only `get` (read), not `sync`, against Argo CD until this is root-caused (check `openshift-gitops-operator` at debug log level, or file/search for a known argocd-operator issue about RBAC reconciliation overriding `spec.rbac` from an operator-side default). **Next-session starting point**: `oc logs -n openshift-gitops-operator deploy/openshift-gitops-operator-controller-manager --tail=0 -f` in one terminal while patching `spec.rbac` in another, to catch the actual reconcile log line in real time (not attempted here due to tooling constraints on concurrent watch + patch in this session).
