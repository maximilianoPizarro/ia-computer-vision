...
**Bug this surfaced along the way**: Authorino's OIDC discovery does *strict* issuer matching -- the `.well-known/openid-configuration` document's `issuer` field must exactly equal the configured `issuerUrl`, or every token gets rejected with `oidc: issuer did not match the issuer returned by provider`. This pattern sets Keycloak's canonical hostname to `sso.<apps-domain>` (`keycloak.ingress.hostname` in `values-hub.yaml`), so JWT `iss` and discovery both say `https://sso.../realms/<r>`. AuthPolicy `issuerUrl` must use that same host (defaults in `oidcpolicy-realms.yaml` / MaaS policies). A second route alias `keycloak.<domain>` may still exist for Developer Hub redirects, but it is **not** the issuer. Separately, Authorino must **trust the OpenShift ingress CA** (`router-ca`) to fetch discovery over HTTPS to `sso.` — without mounting that CA, Authorino logs `tls: failed to verify certificate: x509: certificate signed by unknown authority` / `missing openid connect configuration`, every JWT call 401s, and Swagger UI surfaces it as CORS "Failed to fetch". Pattern fix: `models-as-a-service` `authorino-tls-env` Job builds a CA bundle (system + router-ca + service-ca) and mounts it at `SSL_CERT_FILE=/etc/ssl/certs/openshift-service-ca/service-ca-bundle.crt`.
