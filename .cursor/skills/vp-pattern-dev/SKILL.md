...
### ACM `ConfigurationPolicy` `mustonlyhave` hardcodes and continuously re-enforces the singleton ArgoCD CR's `spec.rbac` (CONFIRMED root cause, hub included)
The published VP `acm` chart (`charts.validatedpatterns.io/acm`, checked through the latest published `0.2.8`) is the actual owner/renderer of the singleton `ArgoCD/vp-gitops` CR when `global.singleArgoCD=true` — **not** `clustergroup-chart` (its `templates/plumbing/argocd.yaml` has an explicit empty `{{- if $.Values.global.singleArgoCD }}{{- else }}...{{- end }}` branch: it renders nothing at all in singleArgoCD mode, deferring entirely to ACM). The `acm` chart's `templates/policies/ocp-gitops-policy.yaml` creates a `Policy`/`ConfigurationPolicy` (`{clusterGroup}-gitops-policy-argocd`) with `remediationAction: enforce` and `complianceType: mustonlyhave` for the full `ArgoCD` CR spec, including a **hardcoded** `spec.rbac: {defaultPolicy: role:readonly, policy: "g, system:cluster-admins/cluster-admins/admin, role:admin", scopes: [groups,email]}` with **no values override key anywhere in the chart** (verified by pulling and reading the chart source directly, not just its README). ACM's `config-policy-controller` re-enforces this exact spec on its own (fast, sub-2-second in practice) evaluation cycle — this applies on the **hub** too, since the hub is itself a member of `clusterGroup.managedClusterGroups`, not just east/west. Any write to `spec.rbac` from anywhere (a local chart's `ServerSideApply=true` Application, `oc patch`, even `clustergroup-chart`'s own `clusterGroup.argoCD.rbac` override key — which is a no-op here since that whole template doesn't render) gets silently reverted within about a second, with no error anywhere. `spec.localUsers` has no equivalent in the ACM policy template, so it is NOT touched and sticks reliably. **Practical impact**: a local user (e.g. `ai-agent` for the Argo CD MCP server) authenticates fine and can `get`/`list` Applications, but falls through to `defaultPolicy: role:readonly` (cannot `sync`) unless bound by name in the hardcoded policy's `g, <group>, role:admin` lines (`system:cluster-admins`, `cluster-admins`, `admin` only). **This is a genuine upstream Validated Patterns framework gap** for the singleArgoCD option (still fairly new — see the 2026-06-04 "New option to use a single ArgoCD instance" VP blog post), not something fixable from a pattern repo today. Mitigation options, roughly in order of preference: (1) wait for/request an upstream override key on the `acm` chart (mirroring how `clustergroup-chart` added `clusterGroup.argoCD.rbac` in v0.9.50 for the non-singleArgoCD path); (2) set the relevant `{clusterGroup}-gitops-policy-argocd` Policy's `remediationAction: inform` out-of-band (breaks GitOps purity, ACM will stop auto-correcting drift on that CR entirely, not just rbac); (3) bind the local user's OIDC/local identity to one of the three hardcoded groups instead of adding new policy lines (not generally possible for a local API-key user). **Verification recipe**: `oc get argocd vp-gitops -n vp-gitops -o yaml --show-managed-fields` and look for field-manager `manager` claiming `spec.rbac` — then `helm pull acm --repo https://charts.validatedpatterns.io --untar` and read `templates/policies/ocp-gitops-policy.yaml` directly rather than trusting the README's documented values keys, which only cover the non-singleArgoCD path.
