...
- **Hub (CONFIRMED, corrected)**: the hub is **not** a member of `clusterGroup.managedClusterGroups` (that value only lists `east`/`west` in `values-hub.yaml`), so **no ACM Policy targets the hub's own `vp-gitops` ArgoCD CR at all** — verified via `oc get application acm -n vp-gitops` resource list (only east/west `Policy`/`Placement`/`PlacementBinding` objects, zero for hub) and via `clusterGroup-chart`'s `templates/plumbing/argocd.yaml`, whose `{{- if $.Values.global.singleArgoCD }}{{- else }}...{{- end }}` branch renders nothing at all in singleArgoCD mode either. On the hub, `oc get argocd vp-gitops -n vp-gitops -o yaml --show-managed-fields` shows a single managedFields entry for field-manager **`manager`** (the **default field-manager name controller-runtime uses when a client doesn't set a custom field owner** — i.e. a plain `client.Update()` call from an operator's reconcile loop, not `ServerSideApply`/`Apply`) that claims the **entire** `spec` (`rbac`, `controller`, `server`, `redis`, `repo`, `sso`, `resourceExclusions`, `resourceHealthChecks`, `resourceTrackingMethod`, `tls.ca`, ...) with `operation: Update`, re-asserted roughly every 15-30s (correlates with `openshift-gitops-operator`'s own "Updating StatefulSet/Deployment ... updating volumes/container resources" reconcile log lines seen at the same cadence). This points at `openshift-gitops-operator` itself doing some form of defaulting/baseline reconciliation of the singleton CR (plausibly a bug where it treats already-set fields as unset and keeps re-applying its own computed default) — **not** ACM, **not** `clustergroup-chart`, and (after removing our own chart's conflicting fields, see below) **not** any chart in this repo either. Not root-caused further within this repo (would need `openshift-gitops-operator` source/issue tracker investigation, out of scope for a pattern repo).
