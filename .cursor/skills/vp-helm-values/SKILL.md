...
29. **`| default true` on a boolean is the single most common footgun in this repo** — Sprig's `default` substitutes whenever the piped value is Go's notion of "empty", and boolean `false` counts as empty, so `{{ if .Values.x.enabled | default true }}` makes an explicit `x.enabled: false` override in a values file a **permanent no-op**, silently re-enabling something a user tried to disable. A repo-wide `grep -rn '| default true' charts/` audit (Jul 2026) found and fixed **20+** instances across more than a dozen charts, every one of them redundant anyway because the chart's own `values.yaml` already defaults the same field to `true` — when you find one, just delete the `| default true`, don't replace it with another default idiom. **Verify the fix, don't just read the diff**: `helm template <chart> --set <flag>=false` and confirm the gated resource is actually absent in the output.
30. **An application's `overrides:` list is replaced wholesale across composed values files, not just base-vs-first-overlay** — when a documented stack combines 3+ `values-hub-*.yaml` overlays and more than one of them overrides the *same* application, the **last-loaded** file's override list is the only one that survives; any override name present in an earlier file but missing from a later file's list for that application is silently dropped. This bit `values-hub-rhpds.yaml` for real (see the `vp-pattern-dev` skill's Jul 5-6 lessons-learned entry for the full live incident — `rhbk-iam.userCount` reverted to its chart default and broke real `ExternalSecret`s). Whenever you edit or add an override for an application that a *different, earlier-loaded* overlay file also overrides, copy every existing override name from that earlier file into your new block too. For the definitive check, don't eyeball the values files — `helm template validatedpatterns/clustergroup -f <every file in the documented stack> --set global.*` and read the generated `Application`'s actual `spec.source.helm.parameters`.
31. **A chart-local hardcoded reference to an optional, disabled-by-default component's namespace breaks every default install** — `charts/all/platform-users`'s `viewNamespacesHub`/`viewNamespacesSpoke` unconditionally listed `stackrox` for its `RoleBinding` template, even though ACS/RHACS is commented out by default in every `values-hub.yaml`/`values-east.yaml`/`values-west.yaml`. Since the namespace never exists, the API server rejects the RoleBinding, and the whole Application stays permanently `OutOfSync`/`Missing` — not a corner case, the *default* install path. Any chart with a static namespace/resource list needs a values flag (`acsEnabled`, etc.) gating entries that correspond to a component this pattern ships disabled by default.
